...
if(len(sys.argv)<3):
	print sys.argv[0],"db_connect_string, path, index_file1, [index_file2, ...]"
	sys.exit(1)
...
class CfgFile:
	...
	def clear_db(self,db):
                #pdb.set_trace()
		db.execute("""delete from cfgfile where md5='%s'"""%(self.md5,))
		db.execute("""delete from cfgfile where filename='%s'"""%(self.cfgname,))
		db.execute("""delete from par_double where md5='%s'"""%(self.md5,))
		db.execute("""delete from par_string where md5='%s'"""%(self.md5,))
	...
	def store(self,db):
		...
		for p in self.dv_double.keys():
			pl=self.dv_double[p]
			for pv in pl:
				db.execute("""insert into par_double(md5,pname,pval) values('%s','%s',%f)"""%(self.md5,p,pv))				
		...
		for p in self.dv_string.keys():
			...
			for pv in pl:
				...
				if(pv):
					#pdb.set_trace()
					print self.md5,p,pv
					q="""insert into par_string(md5,pname,pval) values('%s','%s','%s')"""%(self.md5,p,pv)
					print q
					db.execute(q)				
	...
	def clear_string(self,v):
		...
		# some are too long, more than 4000 characters
		if len(res) > 4000:
			res=res[:3990]+"#TRUNCATED"
		return res	

db_connect_string=sys.argv[1]
connection=cx_Oracle.connect(db_connect_string)
#db.execute("use dbs")
#db.autocommit(0)
...
for i in sys.argv[3:]:
	...
	try:
	   cf=CfgFile(sys.argv[2],i)
	   cf.parse()
	   cf.store(cursor)
	   connection.commit()
        except:
	   print "Problem with index file:"+i
