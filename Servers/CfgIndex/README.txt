...
1. hash values are extracted from DBS with the procedure 
   described in gethash.py Use the following steps with 
   already configured DBS command line interface:
...
2. Get the cfg files from the release. This assumes you 
   have the release set up 
...
4. load into the database. (only works where "path" is "." and you run in directory 
   where the .hash and .index files are located. This loads for mysql, need to write similar one for oracle.
   Syntax: ./cfgindex2db_mysql.py path file1 file2 ... (for oracle use  ./cfgindex2db_oracle.py path file1 file2 ...)
   Example:
   ../cfgindex2db_oracle.py . `ls *.cfg`
