...
class MultiSchema(SchemaInterface) : 
    ...
    def schemaOrder(self) :  
        return self._SchemaOrder
    def schemaInfo(self) : 
        return self._SchemaInfo   # Nary = 0 or 1, FKlist = []
    def schemaNameToSchemaMap(self) :
        self._SchemaNameToSchemaMap   # Map of name to schema
    ...
    def uniqueKeys(self) : 
        ...
        for schemaName in schemasInPK : 
            tmp = []
            for eachpk in self._SchemaNameToSchemaMap[schemaName].primaryKey():
                newpk=schemaName+'.'+eachpk
                tmp.append(newpk)
            ukListByTable[schemaName] = \
                   map(lambda x : map(lambda y : schemaName + '.' + y, x), \
                   self._SchemaNameToSchemaMap[schemaName].uniqueKeys()) + \
                   [tmp]
                   #[schemaName+ self._SchemaNameToSchemaMap[schemaName].primaryKey()]
