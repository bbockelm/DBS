 ...
 * @author sekhri
 $Revision: 1.12 $"
 $Id: DBManagement.java,v 1.12 2007/06/20 21:03:47 sekhri Exp $"
...
public class DBManagement {
	...
	*/
	public static DBManagement getDBConnManInstance() throws Exception {
		...
		synchronized(mutex) {
			if( instance != null ) return instance;
			DBManagement dbm = new DBManagement("jdbc/ms");
			instance = dbm;
		}
		return instance;
	}
...
/**
	...
	private DBManagement() throws Exception {
		try {
			if ( (ds = (DataSource)(((Context)((new InitialContext()).lookup("java:/comp/env"))).lookup("jdbc/ms"))) == null ) {
				throw new SQLException("Datasource cound not be initialized. Connection pooling failed.");
			}
		} catch(javax.naming.NoInitialContextException e) {
			//System.out.println("This must be a standalone client");
