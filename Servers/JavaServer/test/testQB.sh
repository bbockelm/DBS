...
CMD="$JAVA_HOME/bin/java -classpath $CLASSPATH -DDBS_SERVER_CONFIG=$BASE/etc/context.xml dbs.search.parser.Test $savePWD/queries.txt"
#for i in $(cat $savePWD/queries.txt) ; do echo $i; done
$CMD | grep "exception\|PROCESSSING"
