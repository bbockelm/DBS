-- DDL file generated by ./generate_oracle_deployable.sh on 05252007140736
-- ====================================================
-- ====================================================
-- ====================================================
...
PROMPT creating sequence seq_qflagassoc ;
create sequence seq_qflagassoc ;
...
-- ====================================================
-- AUTO INC TRIGGER FOR qflagassoc.ID using SEQ seq_qflagassoc

PROMPT AUTO INC TRIGGER FOR Trigger for Table: qflagassoc
 CREATE OR REPLACE TRIGGER qflagassoc_TRIG before insert on qflagassoc    for each row begin     if inserting then       if :NEW.ID is null then          select seq_qflagassoc.nextval into :NEW.ID from dual;       end if;    end if; end;
/
...
PROMPT LastModified Time Stamp Trigger for Table: qualityhistory
CREATE OR REPLACE TRIGGER TRTSqualityhistory BEFORE INSERT OR UPDATE ON qualityhistory
FOR EACH ROW declare
  unixtime integer
     :=  (86400 * (sysdate - to_date('01/01/1970 00:00:00', 'DD/MM/YYYY HH24:MI:SS'))) - (to_number(substr(tz_offset(sessiontimezone),1,3))) * 3600 ;
BEGIN
...
PROMPT LastModified Time Stamp Trigger for Table: qflagassoc
CREATE OR REPLACE TRIGGER TRTSqflagassoc BEFORE INSERT OR UPDATE ON qflagassoc
FOR EACH ROW declare
  unixtime integer
     :=  (86400 * (sysdate - to_date('01/01/1970 00:00:00', 'DD/MM/YYYY HH24:MI:SS'))) - (to_number(substr(tz_offset(sessiontimezone),1,3))) * 3600 ;
BEGIN
...
INSERT INTO PhysicsGroup (PhysicsGroupName, CreationDate) VALUES ('Trigger', (select (sysdate - to_date('19700101','YYYYMMDD')) * 86400 from dual));
commit;
