...
try:
	...
	if ((op == "both") | (op == "get")) :
		...
		for i in blocks.keys():
			...
			xmlinput = api.getDatasetInfo(path,  blocks[str(i)]['blockName'])
			f = open(name +  blockName + ".xml", "w");
			f.write(xmlinput)
			f.close()
			print "Dataset information fetched from " + args['instance'] + " in XML format is saved in " + name +  blockName +  ".xml"

	if ((op == "both") | (op == "set")) :
		#Insert the saved contents into another DBS instance
		for i in blocks.keys():
			blockName =  blocks[str(i)]['blockName'].replace('/', '_')
			print "Inserting information for Block %s " %  blocks[str(i)]['blockName']
			f = open(name + blockName +  ".xml", "r");
			xmlinput = f.read()
			f.close()
			...
			flog.close()
			print "The transfer log for " + args1['instance'] + " in XML format is saved in " + name + blockName + ".log"
