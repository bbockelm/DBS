...
\Instfoot{cornell}{Cornell, Ithica, NY, USA}
...
\begin{figure}[hbtp]
  \begin{center}
    \resizebox{15cm}{!}{\includegraphics{DBS-WS-DatabasesConceptMap-20060721.eps}}
...
Several use cases for the upcoming CSA06 work are illustrated in Fig.~\ref{fig:csa06-use-cases-diagram} and described in this section.
\begin{figure}[hbtp]
  \begin{center}
    \resizebox{15cm}{!}{\includegraphics{CSA06_use_cases_dfd.eps}}
    \caption{CSA06 use cases diagram.}
...
\begin{itemize}
    ...
    \item Locate a dataset - /primary/tier/processed -> blocks -> SE via DLS 
\end{itemize}
...
Physicist polls a list of available datasets for datasets produced by CSA06. The physicist recognizes Re-RECO data because of the dataset name. The program (or web page) provides a dataset identifier.
 ...
 Data discovery requirements: Identify reRECO - by processed dataset name? 
...
Physicist polls a tool which shows a list of available datasets. He identifies CSA06 jobs by name. [Should be done at CAF, but not for CSA06.]
...
Data discovery requirements: Specify a subset of the RAW data for reRECO (by primary dataset?).
...
He creates data with a set of scripts. He asks X to make file blocks from that data. He asks Y to put that data into the DBS. What tool is used at the tier 0 to insert RAW data into the DBS? How are the primary datasets for real data managed at the tier0? How are primary datasets for MC handled?
...
\begin{figure}[hbtp]
  \begin{center}
    \resizebox{15cm}{!}{\includegraphics{DBS_Indexing.eps}}
...
\begin{figure}[hbtp]
  \begin{center}
    \resizebox{15cm}{!}{\includegraphics{access.eps}}
...
\begin{figure}[hbtp]
  \begin{center}
    \resizebox{15cm}{!}{\includegraphics{schema_07272006.eps}}
...
\item  list$<$parentpathName$>$  = getDatasetProvenance( pathName, list$<$DataTier$>$ )
...
\item  list$<$EventCollections$>$ = getDatasetContents ( pathName ) 
       This provides a list of event collections in a Processed or Analysis 
       Dataset. The list of EventCollections, with the number of events per 
       event collection, should be organized by File Block.
       ...
       (See example in section ~\ref{sec:oldEDM}).

\item  list$<$EventCollections$>$ = getDatasetContents ( initialPathName, predicate ) 
...
\item list$<$FileBlocks$>$ = getDatasetFileBlocks ( pathName )
      ...
      of the getDatasetContents API, so no dedicated API is 
      strictly needed.
