%  $Id: dm-dbs-tech-design.tex,v 1.24 2005/09/28 18:37:37 afanfani Exp $
...
The API could be:
\begin{center}
   list$<$parentpathName$>$  = getDatasetProvenance( pathName, list$<$DataTier$>$ ) 

\end{center}

For example:
\begin{center}
   getDatasetProvenance( /$<$dataset$>$/DST/$<$owner$>$/$<$owner$>$ , [``Hit'',``Digi'',``DST''])
\end{center}
...
\item {\bf get the list of event collection for a DBS ``dataset''} - 
 ...
 job configuration and job splitting. CRAB ask for a DBS dataset, specifying the DBS pathname, 
 and the DBS should provide the list of event collections that correspond 
 to that dataset. 
 The API could be:

\begin{center}
  list$<$EventCollections$>$ = getDatasetContents ( pathName )
\end{center}

 where the list of event collection (with their number of events) will 
 be grouped in term of file-blocks to which the files form the relevant 
 collections belong.

 For example:
 \begin{center}
   getDatasetContents( /$<$dataset$>$/DST/$<$owner$>$/$<$owner$>$)
 \end{center}
 should give as a result something like:

\begin{flushleft}
\mbox{File-Block-1} = \{ \{\mbox{EvC\_Run1},250\}, \{\mbox{EvC\_Run2},235\}, \{\mbox{EvC\_Run5},250\}, ... \}
\end{flushleft}
\begin{flushleft}
\mbox{File-Block-2} = \{ \{\mbox{EvC\_Run15},250\}, \{\mbox{EvC\_Run19},250\}, \{\mbox{EvC\_Run120},245\}, ... \}
\end{flushleft}

 (The data structure here is merely indicative of the appropriate
  content, and not meant to be implemented in exactly that way.)

\item {\bf get the list of fileblocks for a DBS ``dataset''} - 
 CRAB need the list of file-blocks in order to perform file-block 
 location. The file-block location will be done via the Data 
Location Service.
 The list of file-blocks could be extracted from the result 
 of the previous API so no dedicated API is strictly needed.
 Otherwise the API could be:
 \begin{center}
   list$<$FileBlocks$>$ = getDatasetFileBlocks ( pathName )
 \end{center}
